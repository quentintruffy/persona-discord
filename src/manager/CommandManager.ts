/**
 * Gestionnaire de commandes pour un bot Discord
 * Ce module gère le chargement dynamique et l'exécution des commandes Slash
 */
import {
...
export class CommandManager {
  ...
  /**
   * Déploie uniquement les commandes qui ont été modifiées
   * @param guildId - ID facultatif de la guilde (déploiement global si non spécifié)
   * @returns Nombre de commandes déployées
   */
  public async deployUpdatedCommands(guildId?: string): Promise<number> {
    // Cette méthode nécessiterait de comparer les commandes actuelles avec celles déjà déployées
    // Pour simplifier, on redéploie tout pour l'instant
    if (guildId) {
      return this.deployCommandsToGuild(guildId);
    } else {
      return this.deployCommandsGlobally();
    }
  }

  /**
   * Supprime une commande d'une guilde spécifique
  ...
  public async deleteGuildCommand(
    ...
    try {
      ...
      console.log(`✓ Commande ${commandId} supprimée de la guilde ${guildId}`);
      return true;
    } catch (error) {
      this.handleError(
        `Erreur lors de la suppression de la commande ${commandId}`,
        error
      );
      return false;
    }
  ...
  /**
   * Supprime une commande globale
   * @param commandId - ID de la commande à supprimer
   * @returns true si la suppression a réussi
   */
  public async deleteGlobalCommand(commandId: string): Promise<boolean> {
    try {
      if (!this.client.application?.id) {
        throw new Error("L'ID de l'application n'est pas disponible");
      }
      ...
      console.log(`✓ Commande globale ${commandId} supprimée`);
      return true;
    } catch (error) {
      this.handleError(
        `Erreur lors de la suppression de la commande globale ${commandId}`,
        error
      );
      return false;
    }
